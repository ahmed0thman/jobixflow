...
| 2026-08-20 → 2026-08-23 | `../live-system/` (**removed from repo** — see Critical limitation below) | `[LIVE:<path>]` | Behavioral — a real screen-by-screen audit of the current build, not visual-only like the Figma screenshots | Current build behavior for all four dashboards; still quoted directly by `design-plans/00-overview.md` and `design-plans/01-critical-findings-and-conflicts.md`, which predate its removal |
...
**A second, more consequential removal.** Commit `b65fba0` (2026-08-20, "add comprehensive system documentation, design plans, and UI screenshots") added `.claude/docs/live-system/` — a genuine screen-by-screen audit of the *actual running build* across all four dashboards, materially different from `figma-current/`: it documented real field names, edge-case behavior, and data model hints (e.g. an existing `Commission` % field on Add Company, a `Paid By: company/customer` cost field, a live proximity-dispatch map flow), not just visual appearance. Commit `bfbad33` (2026-08-23, "removed old design plans") **deleted the entire folder**, screenshots included.

Unlike `figma-current/`, this removal was **not** accounted for anywhere else in the workspace: `design-plans/00-overview.md` and `design-plans/01-critical-findings-and-conflicts.md` were built by diffing the BRDs against this audit and still cite it directly via `[LIVE:<path>]` tags (e.g. `.claude/docs/live-system/01-platform-admin/02-companies.md`) — those files no longer exist on disk. The content is recoverable from git history (`git show b65fba0:.claude/docs/live-system/<path>`), so nothing is lost, but until it is either restored or the citing design-plans files are re-verified against the live dashboards, treat every `[LIVE:...]` claim in `design-plans/01-critical-findings-and-conflicts.md` (the six numbered findings, including the `Q-02` commission-field evidence and the `ROL-010` proximity-dispatch contradiction) as one step further from the source than its citation implies.
