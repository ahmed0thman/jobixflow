...
import { auditLog as seedAuditLog, companyAdminUser } from '../../data/mock'
...
const PAGE_SIZE = 12
const PRIMARY_KEYS = ['actorRole', 'model', 'action']
...
export function AuditLog() {
  ...
  // Company-scoped: this role only ever certifies operations for its own company,
  // per AUD-001 — no other company's rows are reachable from here at all.
  const myAudit = useMemo(() => seedAuditLog.filter((e) => e.companyId === MY_COMPANY_ID), [])

  const autoFields = useMemo(
    () => inferFilterFields(myAudit, { ...auditLogFilterSchema, exclude: ['companyName'] }),
    [myAudit],
  )
  ...
  }, [myAudit, autoFields, filterValues, datePreset, customFrom, customTo, search])
  ...
  return (
    <div>
      <PageHeader title="Audit Log" subtitle={`Activity for ${companyAdminUser.companyName}`} />
