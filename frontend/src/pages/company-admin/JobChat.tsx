...
import { jobs, companyAdminUser } from '../../data/mock'
...
export function JobChat() {
  ...
  const messages: Record<'main' | 'internal', ThreadMessage[]> = {
    main: mainThread,
    internal: [
      { from: 'Company Dispatcher', text: 'Handing this job off to the next shift.' },
      { from: 'Company Dispatcher', text: 'Acknowledged, taking over now.' },
    ],
  }

  return (
    <div>
      <button onClick={() => navigate(`/company-admin/jobs/${job.id}`)} className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700 mb-4">
        <ArrowLeft className="w-4 h-4" /> Back to {job.displayId}
      </button>

      <PageHeader title={`Chat — ${job.displayId}`} subtitle={job.customerName} />
