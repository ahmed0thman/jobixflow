...
import { jobs, companies, companyAdminUser } from '../../data/mock'
...
export function JobDetail() {
  ...
  if (!job || job.companyId !== companyAdminUser.companyId) {
    return <PermissionDeniedState reason="This job doesn't exist or isn't visible to your company." />
  }
  ...
  return (
    <div>
      <button onClick={() => navigate('/company-admin/jobs')} className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700 mb-4">
        <ArrowLeft className="w-4 h-4" /> Back to Jobs
      </button>

      <PageHeader
        title={job.displayId}
        subtitle={`Created ${formatDateTime(job.createdAt)}`}
        action={
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div className="space-y-5">
          ...
          <Card>
            ...
            <div className="p-5">
              <Button variant="secondary" className="w-full justify-center" icon={<MessageSquare className="w-4 h-4" />} onClick={() => navigate(`/company-admin/jobs/${job.id}/chat`)}>
                Open Chat
              </Button>
            </div>
          </Card>
