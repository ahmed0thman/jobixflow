...
type ThreadMessage = { from: string; text: string; mine?: boolean }
...
export function JobChat() {
  ...
  const [mainMessages, setMainMessages] = useState<ThreadMessage[]>([
    { from: 'Company Dispatcher', text: 'Job received, assigning a technician now.' },
    { from: 'Kailee Reichel', text: 'Thanks — customer says the vehicle is in the driveway.', mine: true },
  ])
  const [internalMessages, setInternalMessages] = useState<ThreadMessage[]>([
    { from: 'Platform Dispatcher (shift handover)', text: 'Heads up — customer called back once already about ETA.' },
  ])
  ...
  function send() {
    ...
    const msg: ThreadMessage = { from: 'Kailee Reichel', text: draft.trim(), mine: true }
  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        subtitle={`${job.customerName} · ${job.companyName || 'Unassigned'}`}
        action={
